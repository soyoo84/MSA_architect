...
async def generate_summary_with_qwen(combined_text: str) -> str:
    ...
    prompt = f"""
    당신은 수석 MSA 아키텍트입니다. 다음은 1,700여 개의 대규모 모놀리식 시스템 파일을 분석하여 추출해 낸 '프로세스 체계(LV1~LV5) 및 도메인(Bounded Context) 매핑 요약 데이터'입니다. 
    이 구조화된 데이터를 바탕으로, 전체 시스템을 한눈에 조망할 수 있는 'To-Be MSA 아키텍처 가이드'를 마크다운 형식으로 작성해 주세요.
    ...
    2. **물리적 Bounded Context 분리 및 정합성 전략**: 대규모 시스템을 위 도메인 단위로 물리적 분리(DB 분리 포함)할 때 적용해야 할 공통 리팩토링 전략 (트랜잭셔널 아웃박스, Saga 패턴, 이벤트 퍼블리싱 등).
    3. **글로벌 UML 도메인 모델 다이어그램**: 핵심 도메인(Bounded Context) 간의 관계와 그 내부의 주요 Aggregate Root들을 보여주는 거시적인 아키텍처 다이어그램을 작성 (반드시 ```mermaid\nclassDiagram\n...``` 형태의 Mermaid 문법 사용).

    [프로세스 체계 및 도메인 압축 데이터]
    {combined_text}
    """
